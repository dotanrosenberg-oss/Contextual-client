...
**Release Label:** Message Edit & Delete
...
- Group participants endpoint - retrieve list of participants with names, phone numbers, admin status, and optional profile pictures
- Message editing via `PATCH /api/customers/:id/messages/:messageId` (text-only, own messages, within WhatsApp's ~15 minute edit window)
- **NEW:** Message deletion via `DELETE /api/customers/:id/messages/:messageId` ("delete for everyone" for any message type sent by the connected account)
- **NEW:** Customer deletion via `DELETE /api/customers/:id` (removes customer from database)
...
Edit a previously sent text message in a WhatsApp group.
...
**Constraints:**
- Only messages sent by the connected account (`isFromMe: true`) can be edited.
- Only text messages can be edited.
- WhatsApp enforces an edit window of approximately 15 minutes after sending.

---

#### DELETE /api/customers/:id/messages/:messageId

Delete a previously sent message from a WhatsApp group. Performs a "delete for everyone" action.

**Response:**
```json
{
  "success": true,
  "messageId": "true_1234567890@g.us_XYZ789"
}
```

**Constraints:**
- Only messages sent by the connected account (`isFromMe: true`) can be deleted.
- All message types (text, media, poll, etc.) can be deleted.
- Group members will see a "This message was deleted" notice.
...
Received when a previously sent message is edited.
...
Received when a previously sent message is deleted.
