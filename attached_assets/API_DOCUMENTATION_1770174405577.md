...
| `adminsApproveNewMembers` | boolean | false | Whether admin approval is required for new members joining via invite link |
...
- `appliedSettings` - The group settings that were applied after creation (only present if settings were requested):
  ...
  - `adminsApproveNewMembers` - Whether admin approval is required for new members
